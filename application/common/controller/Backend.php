    ...
    */
    public function _initialize() {
        ...
        if ($admin['m_type'] == 1) {
            ...
            # 获取列表
            $menu = $this->get_menu($admin['m_structure'], $admin['m_job']);
        }
	...
	*/
    protected function get_menu($structure=0, $job=0){
        ...
        if ($structure == 0) {
            ...
            # 使用菜单ID查询出对应的菜单
            $list = Db::name('menu')->where("m_status = 1 AND m_id in({$menu})")->order('m_id ASC')->field('m_id as id, m_pid as pid,m_name,m_app,m_controller,m_action,m_icon')->select();
        }
        return $list;
    }
